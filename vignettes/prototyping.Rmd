...
own data.  This enables others to use the distributed svd algorithm
implemented in `distcomp` as a function call. 

To install this version of `distcomp`, uninstall any previously
installed versions and then execute:

```{r, eval = FALSE}
##devtools::install_github('hrpcisd/distcomp', ref = "fake_it")
## Moved to master
devtools::install_github('hrpcisd/distcomp')
```

You will need C/C++ compilers to install. 
