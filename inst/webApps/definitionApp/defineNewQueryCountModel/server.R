shinyServer(function(input, output, session) {
    ...
    output$dataLoaded <- reactive({
        if (input$loadData == 0) return()
        ifelse(is.data.frame(getComputationInfo("data")), "Data loaded; Proceed to Query Condition Check", "")
    })

    ## When the user clicks on the "Check Query Condition" button
    ## this function is triggered
    output$checkQueryConditionResult <- renderPrint({
        if (input$checkQueryCondition == 0) return()
        isolate({
            result <- tryCatch(
            {
                setComputationInfo("filterCondition", input$queryCondition)
                setComputationInfo("id", generateId(list(random = runif(10))))
                QueryCountWorker$new(defn = createProjectDefinition(), data = getComputationInfo("data"))
            },
            warning = function(x) x,
            error = function(x) x)
            if ("QueryCountWorker" %in% class(result)) { ## Success
                sprintf("query condition '%s' is OK!", input$queryCondition)
            } else {
                setComputationInfo("filterCondition", NULL)
                setComputationInfo("id", NULL)
                cat("Error!", result$message)
            }
        })
    })

    output$queryConditionChecked <- reactive({
        if (input$checkQueryCondition == 0) return()
        ifelse(is.null(getComputationInfo("filterCondition")), "", "Query condition checked; Proceed to Output Result")
    })
