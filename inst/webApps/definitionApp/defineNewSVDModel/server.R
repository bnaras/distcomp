shinyServer(function(input, output, session) {
  ...
  observe({
    if (input$uploadData > 0 && is.matrix(getComputationInfo("data"))) {
      session$sendCustomMessage('activeNavs', 'Rank Check')
      updateTabsetPanel(session, inputId="navigationList", selected="Rank Check")
    }
  })
  observe({
    if (input$checkRank > 0 && !is.null(getComputationInfo("rank"))) {
  ...
  output$dataUploaded <- reactive({
    if (input$uploadData == 0) return()
    ifelse(is.matrix(getComputationInfo("data")), "Data Uploaded; Proceed to Rank Check", "")
  })
